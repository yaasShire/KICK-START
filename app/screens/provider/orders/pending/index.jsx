...
const PendingOrders = () => {
    ...
    const getPendingOrders = async () => {
        try {
            const { result } = await authorizeProviderGet("booking/getPendingOrders", setError, setLoading)
            console.log(result);
    ...
    const acceptOrder = async () => {
        try {
            ...
            if (result?.message == "The booking has expired and cannot be confirmed") {
                alert("The booking has expired and cannot be confirmed")
            }
            console.log(result);
        } catch (error) {
            console.log(error);
    ...
    return (
        <View style={styles.container}>
            <FlatList
                ...
                renderItem={({ item }) => (
                    <View>
                        <OrderCard data={item} showActionsBtns showAcceptOrder onPress={() => onOrderCardLick(item)} showCancelOrder setSelectedOrder={setSelectedOrder} setAcceptOrderModal={setAcceptOrderModal} setRejectOrderModal={setRejectOrderModal} />
                        <Divider />
                    </View>
                )}
                ListEmptyComponent={() => (
