...
import { Dimensions, FlatList, KeyboardAvoidingView, Platform, Pressable, SafeAreaView, ScrollView, StyleSheet, Text, View } from 'react-native';
...
const EditProfileScreen = ({ route }) => {
    ...
    const onSaveChanges = async (values) => {
        try {
            setLoading(true)
            updateProfileImage()
            const { result } = await authorizedProviderUpdate("authenticate/updateProfileData", setError, setLoading, JSON.stringify(values))
    ...
    const updateProfileImage = async () => {
        try {
            ...
            const { result } = await authorizedProviderUpdate(
